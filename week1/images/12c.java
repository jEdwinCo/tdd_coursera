...
public class TesteCamelCase {
	...
	public void testeNumero() {
		CamelCaseSplit cc_split = new CamelCaseSplit();
		List<String> lista;
		lista = cc_split.converterCamelCase("recupera10Primeiros");
		assertEquals(3,lista.size());
		assertEquals("recupera", lista.get(2));
		assertEquals("10", lista.get(1));
		assertEquals("primeiros", lista.get(0));
	}
	
	@Test(expected=NaoDebeComenzarPorNumeroException.class)
	public void testeNumeroAoInicio() {
		CamelCaseSplit cc_split = new CamelCaseSplit();
		List<String> lista;
		lista = cc_split.converterCamelCase("10Primeiros");
	}
	
	@Test(expected=CaracterNaoAlfaNumerico.class)
	public void testeCaracterNaoAlfaNumerico() {
		CamelCaseSplit cc_split = new CamelCaseSplit();
		List<String> lista;
		lista = cc_split.converterCamelCase("nome#Composto");
	}
